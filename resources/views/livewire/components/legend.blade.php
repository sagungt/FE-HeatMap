...
<div x-data="dataOrdinal"
    class="legend absolute bottom-10 left-5 z-[9999] w-[230px] h-[410px] sm:w-[380px] sm:h-[160px] md:w-[350px] md:h-[160px] lg:w-[300px] lg:h-[384px] duration-700">
    ...
    <div
        ...
        <button x-on:click="animationLegend()"
            class="btn-legend btn-legend-aktif shadow-lg flex justify-center items-center duration-700 md:duration-500 absolute z-[9999] w-[40px] h-[40px] rounded-[14px] bg-[#ffffff] sm:w-[40px] sm:duration-[0.8s] sm:h-[40px] lg:w-[50px] lg:h-[50px]">
        ...
        <div class="flex gap-2 mx-4 lg:gap-2 lg:mx-0  lg:flex-col">
            <button x-on:click="showProperty()" id="show-marker"
                class="text-xs py-1 px-3 text-white duration-300 sm:w-[calc(50%-8px)] lg:w-auto rounded-lg bg-slate-600 hover:bg-slate-600/80 lg:py-2">Show
                markers</button>
            <button x-show="filter.length > 0" x-transition x-on:click="setFilter(showHeatmap())" {{-- x-bind:class="filter.length === 0 ? 'invisible opacity-0' : 'visible opacity-100'" --}}
        ...
        <div class="w-full px-4 md:px-5 lg:px-0">
            <label for="large-range" class="block mb-2 text-sm font-medium text-gray-900"
                x-text="'Opacity ' + Math.round(opacity * 100) + '%'"></label>
            <input x-on:input="changeOpacity($el); opacity = $el.value / 100" id="large-range" type="range"
                min="10" max="100"
